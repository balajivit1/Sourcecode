...
{
    public partial class AdminSentResponsess : Form
    {
        string constring = Convert.ToString(ConfigurationSettings.AppSettings["ConnectionString"]);
        string wholog = "", p1 = "Sent";

        public AdminSentResponsess()
        {
            InitializeComponent();
        }

        private void AdminSentResponsess_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("Select ResID,RCity,RReqby,RName,RExpLocation,RCurLocation,RResUser from ResManagerToAdmin where RResAdmin='" + wholog + "' and RSentStatus='" + p1 + "'", con);
