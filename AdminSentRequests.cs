...
{
    public partial class AdminSentRequests : Form
    {
        string constring = Convert.ToString(ConfigurationSettings.AppSettings["ConnectionString"]);
        string wholog = "", suc = "Success";

        public AdminSentRequests()
        {
            InitializeComponent();
        }

        private void AdminSentRequests_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("Select CID,CReqby,CName,CCity,CExpLocation,CcurLocation,CReqToManager from ComposeRequest where CReqTo='" + wholog + "' and CStatus='" + suc + "'", con);
